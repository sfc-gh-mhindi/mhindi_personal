...
def calculate_mortgage_schedule(
    ...
    while current_loan_balance > 0 and period_counter < max_periods:
        ...
        schedule_data.append({
            ...
            "Starting Loan Balance": starting_loan_balance_this_fortnight, # This will be updated after total_periods is known
        ...
        # Update starting loan balance for the next period (for the 'Starting Loan Balance' column)
        starting_loan_balance_this_fortnight = current_loan_balance # Corrected to use current_loan_balance after reduction
...
if __name__ == "__main__":
    ...
    try:
        loan_amount = float(input("Enter Loan Amount (e.g., 761887): $"))
        
        payment_type = input("Enter Payment Type (Weekly, Fortnightly, or Monthly): ").strip().capitalize()
        if payment_type not in ["Weekly", "Fortnightly", "Monthly"]:
            print("Invalid payment type. Please choose 'Weekly', 'Fortnightly', or 'Monthly'.")
            exit()

        payment_amount = float(input(f"Enter {payment_type} Payment (e.g., 2410): $"))
        duration_years = int(input("Enter Loan Duration in years (e.g., 30): "))
        annual_interest_rate = float(input("Enter Annual Interest Rate (e.g., 6.49 for 6.49%): ")) / 100
        offset_start_balance = float(input("Enter Offset Starting Balance (e.g., 20000): $"))
        offset_growth_per_month = float(input("Enter Offset Growth Per Month (e.g., 2000): $"))
        loan_start_date_str = input("Enter Loan Start Date (DD/MM/YYYY, e.g., 15/08/2025): ")
    ...
    try:
        ...
        # Save to Excel
        output_filename = "mortgage_schedule.xlsx"
        schedule_df.to_excel(output_filename, index=False, float_format="%.2f")
